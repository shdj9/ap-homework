...
class Class:
    ...
    def add_student(self, student: Student):
        self.students[(student.firstname, student.lastname)] = student
    ...
    def load_students_from_file(self, filename: str):
        with open(filename) as file :
            for line in file :
                firstname, lastname = line.rstrip().split(',')
                self.add_student(Student(firstname, lastname))
                
    def load_grades_from_file(self, filename: str): 
        with open(filename) as file :
            for line in file :
                firstname, lastname, topic, *grades = line.rstrip().split(',')
                student = self.get_student(firstname, lastname)
                if student is None : 
                    print(f"student {firstname}, {lastname} does not exist")
                else : 
                    for grade in grades :
    ...
    def compute_averages(self):
        ...
        for student in self.students.keys():
            for topic in Student(student).followed_topics():
                grade = Student(student).compute_average(topic)
                if topic not in averages.keys():
                    averages[topic] = grade
                else : 
                    averages[topic] += grade
        for topic in averages.keys():
            averages[topic] = averages[topic]/catalog[topic]
        return averages
                
    def __repr__(self):
        return f"Class {self.classname} - {self.__len__()} student(s)"
```
